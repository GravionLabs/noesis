...
using Gravion.Contexteur.UseCases.Chunks.GetChunk;
using Gravion.Contexteur.UseCases.Search.SearchDocs;
...
using GetChunkResult = Gravion.Contexteur.UseCases.Chunks.GetChunk.ChunkResult;

namespace Gravion.Contexteur.Server.Tools;
...
public class ContextTools(IMessageBus bus)
{
    ...
    public async Task<string> SearchDocsAsync(
        ...
        string? source = null)
    {
        var result = await bus.InvokeAsync<Result<SearchDocsResult>>(new SearchDocsQuery(query, limit, source));
        if (!result.IsSuccess)
        ...
        string chunkId)
    {
        ...
        var result = await bus.InvokeAsync<Result<GetChunkResult>>(new GetChunkQuery(id));
