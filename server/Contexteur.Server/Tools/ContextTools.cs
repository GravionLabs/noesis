...
{
    [McpServerTool, Description("Search documentation using semantic similarity. Returns relevant text chunks.")]
    public async Task<string> search_docs(
        [Description("The search query")] string query,
        [Description("Maximum number of results to return (default: 5)")] int limit = 5,
        [Description("Optional: filter by source name")] string? source = null)
    {
    ...
    [McpServerTool, Description("Retrieve a specific chunk by its ID.")]
    public async Task<string> retrieve_context(
        [Description("The chunk UUID to retrieve")] string chunkId)
    {
    ...
    [McpServerTool, Description("List all registered documentation sources.")]
    public async Task<string> list_sources()
    {
        var sources = await bus.InvokeAsync<List<Contexteur.Core.Entities.Source>>(new ListSourcesQuery());
        if (sources.Count == 0)
