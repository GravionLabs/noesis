...
Der `.NET`-Server (`Gravion.Noesis.Server`) exposes bereits einen vollständigen MCP-Server:
...
```bash
# Lokal (ohne Docker): dotnet run aus server/src/Gravion.Noesis.Server/
...
Wenn der Server öffentlich erreichbar ist (z.B. `https://noesis.example.com`):

```json
{
  "mcpServers": {
    "noesis": {
      "url": "https://noesis.example.com/mcp"
    }
  }
}
```
...
cd server/src/Gravion.Noesis.Server
dotnet run
